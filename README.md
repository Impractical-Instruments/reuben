...
| `scale-demo` | **yes**           | `sequence` resolved through a tonal context set to C minor — the same degree pattern re-spells live. Change key with `/harmony/root`, reshape with `/harmony/s0`..`s6`. |
