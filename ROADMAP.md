...
- **Math-operator family** — `map`, `add`, `mul`, `differentiate`, `integrate`, … generated by a macro from one `Number`-generic core, mirrored across domains (Message per numeric type; Signal f32). Rich mapping everywhere; calculus ops use real frame-based `dt`. Math is what gestures are made of.
- **Message→Signal converter** — the one sanctioned M→S bridge, a single operator with a `mode` param (snap / slew / smooth / glide). (Signal→Message — envelope-follower machinery — deferred.)
- **One-port-one-type operator sweep** — every functional input becomes one port of one type (Signal where audio-rate is musical, else a Message param); Signal ports carry an unwired default. Retrofit: oscillator `freq` → Signal-only; filter `cutoff`/`resonance` → Signal inputs. Standing authoring rule, added to [docs/agents/authoring.md](docs/agents/authoring.md).
- **Curated control surface (Good Buttons)** — composed from the above (one input fanned to N ranged `map`s over structural addresses), no new JSON section. The first-class *boundary declaration* (public addresses, stable `id`, refactor-safe contract) waits on the nesting/contract thread.
