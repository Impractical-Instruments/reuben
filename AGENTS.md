...
| `reuben-api` | The one window every consumer goes through. Two feature halves — `authoring` (off-thread, serialized) and `render` (the audio callback) — plus the default-off `fs-resolver` reference implementation of the resource seam. |
