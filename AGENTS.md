...
| `reuben-api` | The one window every consumer goes through: the authoring verbs, the engine verbs (swap/control/status/diagnostics) and both ends of the structure channel, plus the tool roster and its advertised prose. Two feature halves — `authoring` (off-thread, serialized) and `render` (the `RenderSide`/`RenderSlot` pair a host drives per block, plus `install_graph` for a host that builds its graph in Rust; `no_std`, and cross-built for bare metal) — plus the resource seam the authoring half calls and the default-off `fs-resolver` reference implementation of it. |
