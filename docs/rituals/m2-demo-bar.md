...
The epic's **terminal** acceptance ritual ([issue #220](https://github.com/Impractical-Instruments/reuben/issues/220)) —
...
(`cargo test -p reuben-core --test m2_swap_harness`) and the ramp-math /
...
swap: M1's restart-swap tore down the cpal streams, rebuilt the Engine, and reopened.
...
new Engine at a block boundary behind a fixed **~20 ms raised-cosine master-gain duck**
and box-transplants survivor nodes across the swap. **There is no stream teardown.**
...
resources) rings straight through it.
...
hanging-note window, not a bug — re-send the note-off; the fixed bass has no gated note,
...
considered and rejected as over-engineering the epic's one intentionally-human
acceptance ritual.
