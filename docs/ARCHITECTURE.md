...
Everything is **addressable** by an OSC path derived from graph structure, plus a curated public control surface an Instrument may expose. Wildcards (`/drums/*/decay`) are designed to dispatch internally as well as externally — which makes Good Buttons and effect racks fall out for free. See [ADR-0005](adr/0005-osc-namespace-and-wildcards.md). *(Wildcards not built yet: today a Message targets at most one node, matched by address prefix. The generated surface exists — per-node `control` metadata → a TouchOSC layout via the `control-surface` skill, [ADR-0018](adr/0018-control-surface-generation.md) — and the first-class boundary declaration landed with nesting and was reshaped into **interface pipes** ([ADR-0034](adr/0034-instrument-nesting.md) §4 as amended by [ADR-0038](adr/0038-interface-pipes-and-the-device-layer.md) §2): an Instrument's `interface` block names its boundary as typed named pipes — an input pipe mints an address internal nodes consume, an output pipe is fed from an internal port — each declaring its own `Arg` type and presentational metadata.)*
