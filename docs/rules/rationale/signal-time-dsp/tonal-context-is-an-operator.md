...
Tonal context is the exact analog of the Clock — polytonality is the polytempo analog — and resolves
the same way ([clock-is-an-operator](clock-is-an-operator.md)). The context **is
...
which a single global could not express. In the engine
