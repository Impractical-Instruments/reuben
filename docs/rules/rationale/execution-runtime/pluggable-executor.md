...
executor interface. The core ships a serial executor and defines that interface; an embedded host
maps the same tasks onto its own job system, and a parallel executor slots in behind the same
interface without the core acquiring a threading policy — the same seam that makes the native I/O
layer detachable ([embed-surface](embed-surface.md),
[single-writer-coordinator](single-writer-coordinator.md)).
