...
already depends on all of core~~ — no longer true, see below) at the cost of another workspace
member, version, and docs surface (the same shape rejected for a `reuben-coordinator` crate). The
