# Why: Execution order is one static topological schedule, recomputed only when the graph changes.
...
independent branches are free to run concurrently. This is the concrete artifact the rule calls the
[Plan](../../execution-runtime.md#plan-lifecycle): a static, topologically ordered execution
schedule.
