# Why: The core and native layer are written in Rust.
...
(`cbindgen`, `godot-rust`, P/Invoke) are first-class, so the core can embed in native, web, and game
shells.
...
the model), and a thinner DSP shelf (acceptable — writing operators *is* the project).
