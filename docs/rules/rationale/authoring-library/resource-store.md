...
Instead it lives in a **`ResourceStore`** the loader/Coordinator builds at load time (the single
writer of structure) and Render reads **immutably** — one store per built graph, with decoded
buffers `Arc`-shared across subpatch reuses and voice copies, the same Coordinator-builds /
Render-reads split the runtime already enforces. A node refers to a resource by a **logical id** in a top-level
...
The accessor is a **pure function of its arguments** — `sample(id, channel, frame)`, one decoded
sample, `0.0` out of range — for one forward-looking reason: the long-term goal is a streaming audio
bank whose total decoded size can exceed RAM. v1.1 keeps every resource resident forever; a future
bank consults a warm-block cache behind the *same* signature, and the operator never re-plumbs.
Rooting ownership in a central store (rather than an operator-owned `Arc<SampleBuffer>`) costs ~one
struct now and is exactly the seam streaming needs — the obvious operator-owned choice would force
the bank to re-plumb every reader. Determinism survives streaming because `sample(id, channel,
frame)` must always return the same float for the same arguments: a bank that falls behind
**underruns** (an xrun, already the device layer's concern) rather than substituting silence, so
streaming affects glitch-vs-no-glitch, never *values*. Data
...
and `reuben-api`'s default-off `fs-resolver` feature fills it with the reference filesystem/WAV
implementation, so compressed formats and non-file sources drop in behind the same trait without
touching core.
