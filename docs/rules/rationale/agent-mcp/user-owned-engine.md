...
rather than gated. Enforcing one-client-per-engine would mean lease/liveness machinery for what is
otherwise a stateless per-call connection, blocking the two-conversation workflow for no demonstrated
need. (A `send` batch is indivisible — one exchange — so no client interleaves into the middle of
another's gesture; that is not arbitration between clients, only atomicity within one.) Structure
