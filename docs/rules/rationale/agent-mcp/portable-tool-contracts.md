...
the native lane serializes, and the **roster identity** is one authority: `reuben_core::tools::CONTRACTS`
declares the name-set and channel kind once, and each door derives its advertised names from it.
Descriptions and input schemas stay per-door — host-flavored, and for MCP carrying rmcp/schemars
machinery core must never depend on.
