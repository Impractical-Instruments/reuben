...
### Resolution ordering (a note for P3/P4) — §5 forces resolver-in-build

A `subpatch` node, unlike every registered operator and unlike the Voicer, has **no static face** — its ports are unknown until the child patch is resolved. So the child must be resolved (via `resolve_instrument`) at **parent-build time**, *earlier* than ADR-0016's pipeline, which resolves resources *after* the `Graph` is built (`parse → build Graph → resolve refs → bind_resources`).

Two placements were originally on the table: thread the `ResourceResolver` into `build`, or run inlining as a post-resolve graph-surgery pass. **§5 constrains the choice**: the synthesized boundary face must exist *during* build pass 2 for the existing wire check to cover boundary wires; a post-build surgery pass would have to carry its own boundary type-check — the second checker §5 forbids. **P4 therefore threads the resolver into `build`.** P3 ([#118](https://github.com/Impractical-Instruments/reuben/issues/118), PR 144) landed recursive resolve + the cycle guard as a post-build pass in `load_instrument` — legitimate there because a P3 `subpatch` presents no ports, so no boundary wire exists to check; that placement is **interim scaffolding P4 relocates** into the build path ([#119](https://github.com/Impractical-Instruments/reuben/issues/119)). The cycle guard itself stays at the `resolve_instrument` seam (it is resolve-time identity, not build logic); only its call site moves.
...
- **Resolution moves earlier for `subpatch`** — the resolver is threaded into `build` (see the ordering note; §5 rules out a post-build surgery pass). `sample`/`voice` resolution is unaffected.
- **Cyclic patch references are fatal** (`LoadError::CyclicResource`), keyed on the source string; diamond reuse is legal (§1).
