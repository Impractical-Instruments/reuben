...
The checked-in fixtures for the M1 milestone's verification harness (ADR-0053 §§1,4,5,6). They sit
...
`../../structure_server.rs` asserts the same channel's behavior field-by-field. (ADR-0053 §1.)
...
(ADR-0046 §10). For a run that is fully self-contained — independent of the evolving instrument
...
ADR-0053 (§§1,4,5,6 — the verification strategy these fixtures implement), ADR-0046 §10 (the M1
restart-swap and its ~100 ms gap), ADR-0048 §5 (the swap report / conflict schema the golden test
pins).
