...
pub enum Arg {
    ...
    // Shared vocab concrete types — each defined once with `#[derive(ArgValue)]`,
...
// `#[derive(ArgValue)]` on each vocab type; they are not written here.
...
/// Pack/unpack a vocab type across the **flat multi-arg OSC form** at the boundary.
///
...
/// A Message an operator emits during `process` onto a Message output port,
...
/// Carries **no address**: internal wires route by connection, and the OSC
...
/// `Arg` into the operator's requested payload type. Carries **no address**: a
...
mod tests {
    ...
    /// `Arg` stays small. Collapsing eight 1-byte vocab-enum variants into one
