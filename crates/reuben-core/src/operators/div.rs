//! `div` — `out = a / b`, per sample.
//!
//! Ratios, normalisation, reciprocal scaling. `b`'s unwired default is the multiplicative
//! identity `1`, so wiring only `a` passes it through unchanged (an unwired Value materializes
//! to its default at the Signal input).
//!
//! Division by zero would yield `±inf`/`NaN` (or panic, for integers) and poison the audio graph, so
//! [`div_fn`] carries an **op-local guard**: a zero divisor produces `0` (a finite, silent result)
//! rather than infinity.
//!
//! - input 0: `a` (`Float`) — the dividend. Unwired default `0`.
//! - input 1: `b` (`Float`) — the divisor. Unwired default `1`; a `0` divisor yields `0`.
//! - output 0: `out` — `a / b` (or `0` when `b == 0`).

/// The op's scalar math, generic over the number type. The `b == 0` check is `div`'s **op-local**
/// guard against an `inf`/`NaN` (or integer divide panic) blowing up the graph; it lives here,
/// inherited by no other op. `Zero` supplies both the zero comparison and the guarded result, so
/// the op works for any numeric type.
#[inline]
fn div_fn<T: num_traits::Zero + core::ops::Div<Output = T>>(a: T, b: T) -> T {
    if b.is_zero() {
        T::zero()
    } else {
        a / b
    }
}

// One declaration -> DivF32Value + DivF32Signal. The multiplicative identity `1` is `b`'s
// unwired default, so wiring only `a` passes it through. Division is not commutative —
// the identity rule holds for the divisor only.
crate::number_operator_contract!(Div {
    variants: [f32 value, f32 signal, i32 value],
    inputs:   { a: number { default 0.0 }, b: number { default 1.0 } },
    outputs:  { out },
    function: div_fn(a, b),
});

#[cfg(test)]
mod tests {
    use super::div_f32_signal::{self, DivF32Signal};
    use super::div_f32_value::{self, DivF32Value};
    use super::div_i32_value::{self, DivI32Value};
    use crate::operators::math_test::{i32_value_emits, signal_out, value_emits};

    /// Drive the signal form; `None` leaves the port unwired (engine materializes its default).
    fn sig(a: Option<&[f32]>, b: Option<&[f32]>, n: usize) -> Vec<f32> {
        signal_out(DivF32Signal::new(), div_f32_signal::OUT_OUT, n, |d| {
            if let Some(a) = a {
                d.drive(div_f32_signal::IN_A, a);
            }
            if let Some(b) = b {
                d.drive(div_f32_signal::IN_B, b);
            }
        })
    }

    /// Drive the value form; returns the emitted quotient(s).
    fn val(a: Option<f32>, b: Option<f32>) -> Vec<f32> {
        value_emits(DivF32Value::new(), |d| {
            if let Some(a) = a {
                d.set(div_f32_value::IN_A, a);
            }
            if let Some(b) = b {
                d.set(div_f32_value::IN_B, b);
            }
        })
    }

    #[test]
    fn divides_two_buffers() {
        let a = [10.0, 20.0, 9.0];
        let b = [2.0, 5.0, 3.0];
        assert_eq!(sig(Some(&a), Some(&b), 3), vec![5.0, 4.0, 3.0]);
    }

    #[test]
    fn unwired_b_is_unity() {
        // Multiplicative identity 1: wiring only `a` passes it through.
        let a = [5.0, 6.0, 7.0];
        assert_eq!(sig(Some(&a), None, 3), vec![5.0, 6.0, 7.0]);
    }

    #[test]
    fn zero_divisor_yields_zero_no_inf() {
        // The op-local guard keeps the graph finite: a/0 -> 0, never ±inf/NaN.
        let a = [1.0, -2.0, 3.0];
        let b = [0.0, 0.0, 0.0];
        let out = sig(Some(&a), Some(&b), 3);
        assert!(out.iter().all(|s| s.is_finite()));
        assert_eq!(out, vec![0.0, 0.0, 0.0]);
    }

    #[test]
    fn divides_held_operands() {
        assert_eq!(val(Some(12.0), Some(4.0)), vec![3.0]);
    }

    #[test]
    fn held_zero_divisor_yields_zero() {
        assert_eq!(val(Some(5.0), Some(0.0)), vec![0.0]);
    }

    /// Drive the `i32` value form; returns the emitted quotient(s) as integers.
    fn val_i32(a: Option<i32>, b: Option<i32>) -> Vec<i32> {
        i32_value_emits(DivI32Value::new(), |d| {
            if let Some(a) = a {
                d.set(div_i32_value::IN_A, a);
            }
            if let Some(b) = b {
                d.set(div_i32_value::IN_B, b);
            }
        })
    }

    // Integer division **truncates toward zero** — this is the one place where instantiating the
    // shared `div_fn` at `i32` gives a materially different answer than at `f32`, rather than the
    // same answer in a different type. Pinned so the difference is documented behaviour an author
    // can rely on, not a surprise.
    #[test]
    fn i32_division_truncates_toward_zero() {
        assert_eq!(val_i32(Some(7), Some(2)), vec![3]);
        assert_eq!(val_i32(Some(-7), Some(2)), vec![-3]);
        assert_eq!(val_i32(Some(12), Some(4)), vec![3]);
    }

    // The op-local zero guard matters more at `i32` than at `f32`: integer division by zero is a
    // panic, where the float form would merely produce `inf` before the guard caught it.
    #[test]
    fn i32_zero_divisor_yields_zero_not_a_panic() {
        assert_eq!(val_i32(Some(5), Some(0)), vec![0]);
    }
}
