...
//!
//! A limiter on CV, a safety rail on a modulated parameter, hard clipping of audio at `±lo/hi`.
//! The bounds `lo`/`hi` follow the carrier like any number operand, so they can be modulated per
//! sample.
//!
...
//! `min(hi)` by hand — an inverted range collapses to `hi` rather than panicking on the hot path.
//!
