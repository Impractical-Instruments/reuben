...
//!
//! Ring-modulation, amplitude scaling, and the VCA (`env.cv -> power -> mul`, audio on the other
//! input). Both operands' unwired default is the multiplicative identity `1`, so wiring only one
//! side passes it through unchanged (an unwired Value materializes to its default at the Signal
//! input).
//!
