//! PowerF32Signal — a unipolar curve shaper, `out = x^exponent`, per sample.
//!
//! Patch it between an `envelope` and a `mul`: `env.cv -> power.x`, `power.out -> mul`, audio ->
//! the other `mul` input.
//!
//! Both carriers are generated by [`number_operator_contract!`](crate::number_operator_contract):
//! `PowerF32Signal` (per-sample buffers) and `PowerF32Value` (held scalars). In the signal form
//! `exponent` is read **per sample** (a uniform buffer operand) rather than once block-rate;
//! functionally identical for a held control (block-slicing re-runs on change).
//!
...
/// integer power would need a separate operator with a separate signature, not this fn at another
/// type.
#[inline]
