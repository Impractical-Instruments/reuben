...
//! **shell**, the module `OPERATORS` census, and a contract-derived `defaults_are_data` test. The alias is
...
impl NumberOpInput {
    fn render(&self) -> TokenStream {
        ...
        let census = self.render_census();
        quote! { #(#rendered)* #census }
    }

    /// The module's `OPERATORS` array — every variant this declaration emitted, as data.
    ///
    /// An `m::*` line in `operator_census!` splices this, so adding a `variants:` entry registers
    /// it without touching the central list. One declaration per module: a second invocation
    /// would define `OPERATORS` twice, which the compiler rejects.
    fn render_census(&self) -> TokenStream {
        let structs = self.variants.iter().map(|v| {
            Ident::new(
                &format!("{}{}{}", self.base, v.name_fragment(), v.carrier.pascal()),
                self.base.span(),
            )
        });
        quote! {
            pub(crate) const OPERATORS: &[crate::registry::OpReg] =
                &[ #( crate::op_reg!(#structs) ),* ];
        }
    }
...
fn parse_variants(input: ParseStream) -> syn::Result<Vec<Variant>> {
    ...
    while !body.is_empty() {
        ...
        // Two entries for the same operator would emit the same module and the same `OPERATORS`
        // element twice — a duplicate-definition error far from its cause.
        if out
            .iter()
...
mod tests {
    ...
    fn binary_op_emits_both_carriers() {
        ...
        // The module census the `abs::*`-style `operator_census!` entry splices: both variants,
        // in declaration order.
        assert!(
            out.contains(
                "pub (crate) const OPERATORS : & [crate :: registry :: OpReg] = \
                 & [crate :: op_reg ! (AddF32Value) , crate :: op_reg ! (AddF32Signal)]"
            ),
            "{out}"
        );
    }
    ...
    // Two entries naming the same operator would emit the same module twice — caught here rather
    // than as a duplicate-definition error far from its cause.
    #[test]
