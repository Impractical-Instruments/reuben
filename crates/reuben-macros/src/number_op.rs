    ...
    fn render_variant(&self, number: &Ident, carrier: Carrier) -> syn::Result<TokenStream> {
        ...
        Ok(quote! {
            pub mod #mod_ident {
                ...
                // `Default` has no caller — neither here nor on any hand-written operator. It is
                // there for parity: `clippy::new_without_default` demands it of every operator in
                // `reuben-core` (the sole exception, `pipe`, escapes only because its `new` takes
                // an argument), but clippy suppresses its own lints inside another crate's macro
                // expansion, so it never fires here. Dropping it would make the generated families
                // the only operators without `Default`, with nothing to flag the divergence.
                #[derive(Default)]
