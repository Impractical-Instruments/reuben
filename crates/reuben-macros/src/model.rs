//! The pure core of `operator_contract!`: turn a validated [`OperatorSpec`] into a
...
pub struct ContractModel {
    ...
    /// Instantiate-time **`Constant`** ports, numbered with `C_` index consts. Empty for
    /// the common operator.
    pub constants: Vec<PortModel>,
    pub resources: Vec<String>,
}

/// Number a set of ports sequentially in declaration order: inputs are indexed `0..`,
...
mod tests {
    ...
    // Ports number sequentially in declaration order: the former per-kind split is
    ...
    // old `params` profile — runtime controls are inputs now).
    #[test]
