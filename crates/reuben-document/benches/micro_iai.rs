...
//!
//! iai's `#[bench::…]` list is compile-time, so it can't iterate the registry (whose entries are
//! `fn` pointers, readable only at runtime). **This file is therefore the one
