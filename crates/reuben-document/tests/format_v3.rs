...
use reuben_document::format::LoadWarning;
use reuben_document::resources::{ResolveError, ResourceResolver};
use reuben_document::{load_instrument, NormalizedDoc};
...
fn control_block_carrying_doc_renders_bit_identical_to_the_stripped_doc() {
    // `control` was an opaque passthrough the engine never read, so dropping it is render-safe
