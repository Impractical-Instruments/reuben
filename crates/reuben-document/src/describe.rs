...
/// which the built [`Graph`](reuben_core::graph::Graph) deliberately does not (they are
...
pub fn describe_boundary(doc: &InstrumentDoc, loaded: &Loaded) -> BoundaryDesc {
    let g = &loaded.graph;
    let port_desc = |name: &String, key: reuben_core::graph::NodeKey, idx: usize, output: bool| {
